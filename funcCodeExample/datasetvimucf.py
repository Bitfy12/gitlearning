...
class VideoDataset(Dataset):
    ...
    def load_frames(self, file_dir):
        ...
        for i, frame_name in enumerate(frames):
            ...
            if frame.shape[0] != 344 or frame.shape[1] != 256:
                frame = cv2.resize(frame, (344, 256), interpolation=cv2.INTER_LINEAR)

            buffer[i] = frame

        # if buffer.shape[0] > self.clip_len:
        #     # 如果帧数超过 clip_len，则随机裁剪
        #     start_idx = np.random.randint(buffer.shape[0] - self.clip_len)
        #     buffer = buffer[start_idx:start_idx + self.clip_len]
        # elif buffer.shape[0] < self.clip_len:
        #     # 如果帧数少于 clip_len，则用最后一帧填充
        #     last_frame = buffer[-1].copy()
        #     padding = np.tile(last_frame, (self.clip_len - buffer.shape[0], 1, 1, 1))
